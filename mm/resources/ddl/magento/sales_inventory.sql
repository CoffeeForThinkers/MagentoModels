 ...
 LEFT OUTER JOIN (SELECT item.entity_id AS product_id, 
                   ...
                   FROM catalog_product_entity_varchar AS item 
                     INNER JOIN eav_attribute AS att ON item.attribute_id = att.attribute_id 
				      WHERE att.attribute_code = 'style'
				      LIMIT 1) AS att_style ON att_style.product_id = sales_items.product_id
WHERE sales_items.product_type = 'simple' AND 
      sales.`status` NOT IN ('canceled', 'fraud', 'holded', 'paypal_canceled_reversal', 'paypal_reversed', 'pending_payment', 'pending_paypal') AND 
      sales.created_at BETWEEN date_from AND date_to
;
